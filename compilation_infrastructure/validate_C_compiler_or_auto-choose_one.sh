...
test_alleged_C_compiler() {
  ...
  test_file_base_pathname=$prefix/C_compiler_validation_test_started_at___`date '+%a_%A_%b_%B_%C_%d_%H_%I_%j_%m_%M_%p_%S_%u_%U_%V_%w_%W_%y_%Y_%Z'`
...
if test -n "$compiler_command" && is_executable_and_not_a_directory "$compiler_command" && test_alleged_C_compiler "$compiler_command"; then # if the alleged compiler arg. is not provided, or points to something not executable or a directory
  ...
  for alleged_compiler_command in gcc_latest cc gcc clang; do
                                # ^^^^^^^^^^ a kludge, at least on Abe`s "Zaphod" when running Debian 7, to work around Abe`s workaround [everybody got that? ;-)] for the fact that the GCC folks screwed up such that building the Linux kernel with GCC > 6.5.0 is broken [at least when targeting AMD64]
    alleged_compiler_fullPath=`which $alleged_compiler_command`
    if is_executable_and_not_a_directory "$alleged_compiler_fullPath" && test_alleged_C_compiler "$alleged_compiler_fullPath"; then
